...
PIDEngine::PIDEngine()
{
  ...
  m_speed_factor = 20.0;
  m_current_time = 0;
  ofs.open ("test1.txt", std::ofstream::out | std::ofstream::app);
}
				     ...
				     double max_elevator)
{
  ...
  //double pitch_error = 0 - desired_pitch;
  ...
  float a = desired_elevator;
  // Enforce elevator limit
  MOOSAbsLimit(desired_elevator,max_elevator);
  
  ofs<<depth_error<<" "<<current_pitch<<" "<<desired_pitch<<" "<<pitch_error<<" "<<desired_elevator<<" "<<a<<" "<<max_elevator<<endl;
