...
bool MarinePID::OnNewMail(MOOSMSG_LIST &NewMail)
{
  ...
  for(p=NewMail.begin(); p!=NewMail.end(); p++) {
	...
	else if(key == "PID_SETTINGS")
		getNewSettings(msg.m_sVal);
      }
  }
  return(true);
}
...
void MarinePID::registerVariables()
{
  ...
  Register("CONTROLLER_ACTIVE", 0);
  Register("PID_SETTINGS", 0);
}
...
void MarinePID::getNewSettings(string msg)
{
	...
	for(int i=0;i<vectorOfKeyValues.size();i++)
		...
		if(keyValue[0] == "pitchP")
			pitch_pid_Kp = atof(keyValue[1].c_str());
		else if(keyValue[0] == "pitchD")
			pitch_pid_Kd = atof(keyValue[1].c_str());
		else if(keyValue[0] == "pitchI")
			pitch_pid_Ki = atof(keyValue[1].c_str());
		else if(keyValue[0] == "pitchILim")
			pitch_pid_ilim = atof(keyValue[1].c_str());
		else if(keyValue[0] == "zpitchP")
			z_top_pid_Kp = atof(keyValue[1].c_str());
		else if(keyValue[0] == "zpitchD")
			z_top_pid_Kd = atof(keyValue[1].c_str());
		else if(keyValue[0] == "zpitchI")
			z_top_pid_Ki = atof(keyValue[1].c_str());
		else if(keyValue[0] == "zpitchILim")
			z_top_pid_ilim = atof(keyValue[1].c_str());
		}
		
	//cout<<pitch_pid_Kp<<" "<<pitch_pid_Kd<<" "<<pitch_pid_Ki<<endl;
	ScalarPID ztopPID;
	ztopPID.SetGains(z_top_pid_Kp, z_top_pid_Kd, z_top_pid_Ki);
	ztopPID.SetLimits(z_top_pid_ilim, 100);
	m_pengine.setPID(2, ztopPID);
