...
bool setUIntOnString1(unsigned int& given_val, string str)
{
  if(!isNumber(str))
    return(false);
...
ObstacleManager::ObstacleManager()
{
  ...
  m_depth_threshold_avoid = DEPTH_THRESHOLD_AVOID;
  m_dist_threshold_add_point = DIST_THRESHOLD_ADD_POINT;
  m_depth_threshold_add_point = DEPTH_THRESHOLD_ADD_POINT;
...
bool ObstacleManager::OnNewMail(MOOSMSG_LIST &NewMail)
{
  ...
  for(p=NewMail.begin(); p!=NewMail.end(); p++) {
    ...
    if(key == m_point_var)
	  ...
	  myfile<<"m_point_var:"<<m_point_var<<"="<<sval<<endl;
	  }
    if(key == "NAV_X") {
      m_nav_x = dval;
      handled = true;
    ...
    else if(key == "OBJ_ALERT_REQUEST") 
		{
		handled = handleMailAlertRequest(sval);
		myfile<<"OBJ_ALERT_REQUEST="<<sval<<endl;
		}
...
bool ObstacleManager::OnStartUp()
{
  ...
  for(p=sParams.begin(); p!=sParams.end(); p++) {
    ...
    else if(param == "depth_threshold_avoid")
      handled = setPosDoubleOnString(m_depth_threshold_avoid, value);
    else if(param == "dist_threshold_add_point")
      handled = setPosDoubleOnString(m_dist_threshold_add_point, value);
    else if(param == "depth_threshold_add_point")
      handled = setPosDoubleOnString(m_depth_threshold_add_point, value);
  ...
  if(m_map_poly_convex.size() > 0)
	{
	  ...
	  for(p=m_map_poly_convex.begin(); p!=m_map_poly_convex.end(); p++) {
		...
		if(poly.is_convex()) {
		  double dist = poly.dist_to_poly(x, y);
		  if(dist <= m_dist_threshold_add_point && (fabs(m_map_max_depth[obs_key]-z) < m_depth_threshold_add_point || fabs(m_map_min_depth[obs_key]-z) < m_depth_threshold_add_point))
			obstacle_key_str = obs_key;
		  else
			obstacle_key_str = "obstacle" + intToString(m_map_poly_convex.size());
		}
	  }
	}
	else
		obstacle_key_str = "obstacle0";
	
  myfile<<"przeszkoda="<<obstacle_key_str<<endl;
...
bool ObstacleManager::handleMailNewPoint(string value)
{
  ...
  myfile<<"!!!!!!!!!!!!!!!!!!!!!!!!1XYPoint="<<newpt.get_spec()<<endl;
  if(!newpt.valid()) {
    m_points_invalid++;
...
//#if 1
  ...
  if(m_map_points[obstacle_key].size() > 0) {
	...
	if(m_map_max_depth[obstacle_key] < newpt.z())
		m_map_max_depth[obstacle_key] = newpt.z();
	
	if(m_map_min_depth[obstacle_key] > newpt.z())
		m_map_min_depth[obstacle_key] = newpt.z();
  }
  else
	{
	m_map_max_depth[obstacle_key] = newpt.z();
...
myfile<<"5"<<endl;
  ...
  if(m_map_points[obstacle_key].size() == 1)
	{
	XYPoint virtualPoint1(newpt.x()+0.1*rand()/RAND_MAX, newpt.y()+0.1*rand()/RAND_MAX);
	virtualPoint1.set_vz(newpt.z());
	XYPoint virtualPoint2(newpt.x()+0.1*rand()/RAND_MAX, newpt.y()+0.1*rand()/RAND_MAX);
	virtualPoint2.set_vz(newpt.z());
	m_map_points[obstacle_key].push_back(virtualPoint1);
	m_map_points[obstacle_key].push_back(virtualPoint2);
	myfile<<virtualPoint1.get_spec()<<" "<<virtualPoint1.get_spec()<<endl;
	}
...
bool ObstacleManager::handleMailAlertRequest(string request)
{
  ...
  myfile<<"Na razie OK"<<endl;

  if((update_var == "") || (alert_range == ""))
    return(false);
...
void ObstacleManager::postConvexHullUpdates()
{
  ...
  if(m_alert_var == "")
    return;
//cout<<"postConvexHullUpdates"<<endl;
  map<string, XYPolygon>::iterator p;
  // For all obstacles that have a convex hull
  for(p=m_map_poly_convex.begin(); p!=m_map_poly_convex.end(); p++) {
    string obs_key = p->first;
    ...
    if(poly.is_convex()) {
      ...
      if(dist <= m_alert_range && (fabs(m_map_max_depth[obs_key]-m_nav_z) < m_depth_threshold_avoid || fabs(m_map_min_depth[obs_key]-m_nav_z) < m_depth_threshold_avoid))
        postConvexHullUpdate(obs_key);
		
	
	//myfile<<"OBM_DIST_TO_OBJ_" + toupper(obs_key)<<" "<<dist<<" "<<fabs(m_map_max_depth[obs_key]-m_nav_z)<<" "<<fabs(m_map_min_depth[obs_key]-m_nav_z)<<endl;
	//myfile<<"OBM_DIST_TO_OBJ_" + toupper(obs_key)<<" "<<m_map_max_depth[obs_key]<<" "<<m_map_min_depth[obs_key]<<" "<<m_nav_z<<endl;
    }
  }
}
...
void ObstacleManager::postConvexHullUpdate(string obstacle_key)
{
  ...
  //string update_str = "name=" + obstacle_key + "#poly=" + poly.get_spec_pts(5) + ",label=" + obstacle_key;
  string update_str = "poly=" + poly.get_spec_pts(5) + ",label=" + obstacle_key;
  update_str +="&min_depth=" + doubleToString(m_map_min_depth[obstacle_key]) + ",max_depth=" + doubleToString(m_map_max_depth[obstacle_key]);
  myfile<<"postConvexHullUpdate!!!!!"<<endl;
  myfile<<m_alert_var<<" "<<update_str<<endl;
