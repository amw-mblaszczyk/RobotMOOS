...
BHV_NewWaypoint::BHV_NewWaypoint(IvPDomain gdomain) : 
  IvPBehavior(gdomain)
{
  // First set variables at the superclass level
  m_descriptor = "BHV_NewWaypoint";  
  m_domain     = subDomain(m_domain, "course,speed");

  // Set behavior configuration variables
  m_cruise_speed     = 0;  // meters/second
  m_cruise_speed_constant = 0;
  m_max_vehicle_speed = 2;
  ...
  addInfoVars("NAV_X, NAV_Y, NAV_SPEED, NAV_SPEED_OVER_GROUND");
  m_markpt.set_active(false);
  m_markpt.set_vertex_size(4);

  m_prevpt.set_label("prev_pt");
  
  //m_start_mission = false;
  
  //m_timeP = 2;
  m_currentP = 2;
  m_threshold_for_error_track = 0.5;
  
  m_dist_to_trackline = 0;
  
  //wsp_temp = 1;///////////////////////////////////////!!!!!!!!!!!!!!!!1
  //ofs.open ("test.txt", std::ofstream::out | std::ofstream::app);/////!!!!!!!!!!!!!!!!!!!1
}

//-----------------------------------------------------------
// Procedure: onSetParamComplete()

void BHV_NewWaypoint::onSetParamComplete()
{
...
bool BHV_NewWaypoint::setParam(string param, string param_val) 
{
  ...
  if((param == "polygon") || (param == "points")) {
   ...
   else if(param == "max_vehicle_speed")  {
    if(dval > 0)
		m_max_vehicle_speed = dval;
    return(true);
  }
  
  else if(param == "pid_current_p")  {
    if(dval > 0)
		m_currentP = dval;
    return(true);
  }
  
  else if(param == "m_threshold_for_error_track")  {
    if(dval > 0)
		m_threshold_for_error_track = dval;
    return(true);
  }
...
void BHV_NewWaypoint::onIdleToRunState() 
{
  if(!updateInfoIn()) 
    return;
  m_odo_leg_disq = true;
...
void BHV_NewWaypoint::onRunToIdleState() 
{
  postErasables();
...
BehaviorReport BHV_NewWaypoint::onRunState(string input) 
{
...
IvPFunction *BHV_NewWaypoint::onRunState() 
{
  /*if(!m_start_mission)
	{
	m_starting_point.set_vertex(m_osx, m_osy);
	m_start_mission = true;
	}*/
...
void BHV_NewWaypoint::onIdleState() 
{
  if(!updateInfoIn()) 
    return;
}
...
bool BHV_NewWaypoint::updateInfoIn()
{
  ...
  m_os_sog = getBufferDoubleVal("NAV_SPEED_OVER_GROUND",  ok5);
...
bool BHV_NewWaypoint::setNextWaypoint()
{
  ...
  if(m_lead_distance >= 0) {
    ...
    if(track_anchor) {
	  ...
	  m_perp_point.set_vx(nx);
	  m_perp_point.set_vy(ny);
      //XYPoint perp_pt(nx, ny);
      
      double damper_factor = 1.0;
      if(m_lead_damper > 0) {
	m_dist_to_trackline = hypot((nx-m_osx),(ny-m_osy));
	if(m_dist_to_trackline < m_lead_damper) {
	  double augment = 1 - (m_dist_to_trackline / m_lead_damper);
	...
	dist = m_lead_distance * damper_factor;  
      ...
      m_trackpt.projectPt(m_perp_point, angle, dist);
    }
  }
...
void BHV_NewWaypoint::setNewSpeed()
{
	if(m_dist_to_trackline > m_threshold_for_error_track)
		m_cruise_speed = m_cruise_speed_constant + m_currentP*m_dist_to_trackline;
	else
		{
		float diff = m_os_sog - m_cruise_speed_constant;
		m_cruise_speed = m_osv - diff;
		}
	
	if(m_cruise_speed > m_max_vehicle_speed)
		m_cruise_speed = m_max_vehicle_speed;
}
...
IvPFunction *BHV_NewWaypoint::buildOF(string method) 
{
  IvPFunction *ipf = 0;
...
void BHV_NewWaypoint::postStatusReport()
{
  int    current_waypt = m_waypoint_engine.getCurrIndex();
...
void BHV_NewWaypoint::postViewableSegList()
{
...
void BHV_NewWaypoint::postErasables()
{
  postMessage("VIEW_POINT", m_trackpt.get_spec("active=false"), "trk");
  postMessage("VIEW_POINT", m_nextpt.get_spec("active=false"), "wpt");
...
void BHV_NewWaypoint::postCycleFlags()
{
...
void BHV_NewWaypoint::postWptFlags(double x, double y)
{
...
void BHV_NewWaypoint::handleVisualHint(string hint)
{
...
void BHV_NewWaypoint::updateOdoDistance()
{
  if(!m_odo_virgin) {
...
void BHV_NewWaypoint::markOdoLeg()
{
